...
# Load OPENROUTER_API_KEY / OPENROUTER_MODEL from the project .env so the key is
# pre-filled in the sidebar for the demo.
load_dotenv()
...
class StoryState(TypedDict):
    ...
    pending_choice: Optional[str]   # the just-clicked choice id, carried across reruns
...
class AIStoryGenerator:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow.add_edge(START, "initialize_story")
...
def main():
    ...
    with st.sidebar:
        ...
        api_key = st.text_input(
            "OpenRouter API Key",
            type="password",
            value=os.getenv("OPENROUTER_API_KEY", ""),  # pre-filled from .env
